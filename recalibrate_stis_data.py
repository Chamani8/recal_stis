...
def run_x1d(sci_infile, x1d_save_path, prod_type = "drj", out_prod_type = "sx1"):
    infiles = glob.glob(f"{sci_infile}*{prod_type}.fits")
    ...
    delete_files([f"{prod_type}_x1d.fits"], file_loc=sci_infile)
    ...
    if x1d_save_path != None:
        command_args = ["cp", f"{x1d_outfile}", f"{x1d_save_path}{rootname}_{out_prod_type}.fits"]
        subprocess.run(command_args)
        print(f"1D spectra fits file copied to:   {x1d_save_path}{rootname}_{out_prod_type}.fits")
...
if __name__ == "__main__":
    ...
    main(args.sci_infile, args.flat_infile, args.x1d_save_path)
